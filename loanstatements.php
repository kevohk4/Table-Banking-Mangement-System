...
<body>
    ...
    <div id="introduction">
        ...
        </tr><tr><td>Account number:".$row['member_id']."</td>
        </tr>
        </table>";
        ?>
        <div class="navig">
        ...
        <td><button onclick="document.getElementById('id01').style.display='block'" style="width:auto;" id="1d04">Loan bal</button></a></td>
        ...
        <?php
             ...
             $records = mysqli_query($con,$sql);
                ...
                $currentBal=$row['current_bal'];
                     ...
                     echo "<p>Loan repayement <a href='repayment1.php'>Repay Loan</a></p>"; }
                else{}
          ?>
    </div>
    </div>
    <div id="middle">
        ...
        <?php     
            $result = mysqli_query($db,"SELECT * FROM loans where trans_By='$login_session' order by date DESC");
            while($row = mysqli_fetch_array($result)){
                ...
                $formated_prevBal=number_format($prevbal,2);
                echo '<td>Ksh '.$formated_prevBal.'</td>';
                $amount=$row['paid'];
                $formated_amount=number_format($amount, 2);
                echo '<td>Ksh '.$formated_amount. '</td>';
                $currentBal=$row['current_bal'];
                $formated_currBal=number_format($currentBal,2);
                echo '<td>Ksh '.$formated_currBal.'</td>';
                        echo "</tr>";
            }
            ?>
